...
def _flat_series(value: float) -> FetchSeriesFn:
    levels = {
        RATE10Y_SERIES["JPY/USD"]: 2.0,
        RATE10Y_SERIES["JPY/EUR"]: 0.5,
        JPY_RATE10Y_SERIES: 0.1,
    }
...
def _cpi_series() -> FetchSeriesFn:
    rates = {
        CPI_SERIES["JPY/USD"]: 0.002,
        CPI_SERIES["JPY/EUR"]: 0.0005,
        JPY_CPI_SERIES: 0.0001,
    }
...
def test_unmapped_symbol_rejected_for_term_carry(tmp_path: Path) -> None:
    ...
    with pytest.raises(FactorError, match="JPY/SGD"):
        add_term_carry(
