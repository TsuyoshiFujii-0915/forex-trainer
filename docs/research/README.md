...
- **現時点の最良エージェント**: クロスセクション特徴量(xz/xr_mom24)を与えた PPO の
  3シード行動平均アンサンブル — walk-forward 2019〜2025 で**ネット +2.5%/年、
  グロス +5.4%/年、Sharpe +0.46**(ルールベンチマーク超え)。
- ただし 2024年への依存が大きく、未使用ホールドアウト(2026Q2 相当、63決定)は
  ほぼフラットで不確定。**「統計的に有意なプラス」の主張にはまだ証拠不足** —
  時間足化・ユニバース拡張によるサンプル増が次の主要課題。
